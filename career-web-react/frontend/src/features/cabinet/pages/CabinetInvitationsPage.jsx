...
// Espace Cabinet › Invitations : inviter des recruteurs (une adresse ou import
// en masse), suivre les invitations et les sièges de licence disponibles.
import { AdminPageLoader } from "../../../components/AdminPageLoader.jsx";
import { getFriendlyErrorMessage } from "../../../lib/errors.js";
import { formatDateTime } from "../../../lib/format.js";
import { getCabinetInvitations, getCabinetOverview, sendCabinetInvitation, sendCabinetInvitationsBulk } from "../../../lib/inMemoryDb.js";
...
export default function CabinetInvitationsPage({ user, language }) {
  ...
  async function submitSingle() {
    ...
    setDialogError("");
    try {
      await sendCabinetInvitation(user.id, email.trim());
      setInviteOpen(false);
      reload();
  ...
  async function submitBulk() {
    ...
    setBulkResult(null);
    try {
      const result = await sendCabinetInvitationsBulk(user.id, bulkEmails);
    ...
    <section className="admin-accounts jy-invitations">
      <header className="module-header admin-accounts-header">
        <div>
          ...
          <p>{t("Invitez vos recruteurs par e-mail : un siège de votre licence leur est réservé automatiquement.", "Invite your recruiters by email: a seat on your license is reserved automatically.")}</p>
        </div>
          ...
          <span className="jy-seat-banner-text">
            ...
            <small>
              {seatsLeft === 0
                ...
                : t(`${seatsLeft} siège(s) encore disponible(s) pour de nouveaux recruteurs.`, `${seatsLeft} seat(s) still available for new recruiters.`)}
            </small>
          </span>
      ...
      <div className="admin-table-toolbar">
        ...
        <div className="jy-list-tools">
          <AdminExportMenu language={language} title={t("Invitations", "Invitations")} fileBase="invitations-recruteurs" columns={exportColumns} rows={filtered} />
        </div>
      </div>
      ...
      {invitations.length ? (
        <>
          <div className="admin-table-wrap">
            <table className="admin-table">
              <thead>
                <tr>
                  <th>{t("Recruteur invité", "Invited recruiter")}</th>
        ...
        <div className="jy-card jy-empty-block">
          ...
          <span>{t("Invitez vos recruteurs un par un, ou toute une équipe d'un coup avec l'import en masse.", "Invite recruiters one by one, or a whole team at once with bulk import.")}</span>
      ...
      <JyDrawer
        ...
        subtitle={t("Invitation recruteur", "Recruiter invitation")}
                ...
                },
                {
                  ...
                  content: (
                    <p className="jy-drawer-empty">
                      {isAccepted(selected)
                        ? t("Le recruteur a créé son compte avec ce code : il apparaît dans le module Recruteurs et partage le vivier.", "The recruiter created their account with this code: they appear in the Recruiters module and share the pool.")
                        : t("Le recruteur a reçu un e-mail avec le code de licence. Dès qu'il crée son compte « Recruteur interne » avec ce code, il rejoint votre cabinet.", "The recruiter received an email with the license code. As soon as they sign up as an “Internal recruiter” with it, they join your firm.")}
                    </p>
                  )
                }
              ]
      ...
      <MfaDialog
        ...
        icon="send"
        title={t("Inviter des recruteurs", "Invite recruiters")}
        description={
          seatsLeft != null
      ...
      >
        ...
        {mode === "single" ? (
          <label className={`mfa-field ${emailTouched && emailError ? "has-error" : ""}`}>
            <span>{t("E-mail du recruteur", "Recruiter email")}</span>
            <input
              type="email"
              autoFocus
              value={email}
            ...
            {emailTouched && emailError ? (
              ...
              <small>{t("Le recruteur reçoit un e-mail avec le code de licence de votre cabinet.", "The recruiter receives an email with your firm's license code.")}</small>
            )}
          </label>
        ) : bulkResult ? (
          ...
          <>
            ...
            <label className="mfa-field">
              ...
              <textarea rows={5} value={bulkText} placeholder={"jean.dupont@cabinet.fr\nmarie.martin@cabinet.fr"} onChange={(event) => setBulkText(event.target.value)} className="jy-textarea" />
