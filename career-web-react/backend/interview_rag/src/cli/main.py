...
from src.generation.interview import END_INTERVIEW_MESSAGE, KICKOFF_MESSAGE, run_turn
...
def main() -> None:
    ...
    print("L'assistant joue le rôle du recruteur. Réponds aux questions posées.")
    print("Tape 'fin' ou 'bilan' pour clore l'entretien et obtenir ta correction globale.")
    print("Tape 'quit' pour quitter à tout moment.\n")
    ...
    while True:
        ...
        is_end_request = candidate_message.lower() in {"fin", "bilan"}
        msg_to_send = END_INTERVIEW_MESSAGE if is_end_request else candidate_message

        try:
            answer = run_turn(msg_to_send, history, type_entretien, domaine, offre)
        except Exception as exc:
            print(f"\nErreur : {exc}\n")
            continue
