...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        try:
            ...
            elif self.path == "/api/audio-message":
    ...
    def _handle_audio_message(self) -> None:
        length = int(self.headers.get("Content-Length", 0))
        if not length:
            self._send_json({"error": "Fichier audio vide."}, status=400)
            return
        ...
        transcribed_text = transcribe_audio(audio_bytes, filename=filename)
        if not transcribed_text:
            self._send_json({"error": "Aucune parole détectée dans l'enregistrement."}, status=400)
            return
        ...
        self._send_json({"transcribed_text": transcribed_text, "message": answer})
